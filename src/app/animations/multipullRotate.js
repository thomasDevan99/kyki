...
export default function MultipullRotate({ setisIdle, numberOfPulls }) {
  ...
  // Reset the portal and button states
  const reset = () => {
    setFlipped(false);
    setisIdle(true);
  };
  ...
  return (
    <>
      ...
      <div className="relative w-full h-[80vh] flex items-center justify-center overflow-hidden">
        ...
        >
          {flipped ? "PULL" : "Center"}  {/* Change text based on flip state */}
        </motion.button>
      </div>

      {/* Action Buttons (Pull Again & Reveal All) */}
      <div className="sm:items-center justify-items-center grid grid-cols-2 gap-10">
        {(
          <motion.button
            ...
            onClick={() => reset()}
          >
            <div className="p-4">Pull Again</div>
          </motion.button>
        )}
        {(
          <motion.button
            className="rounded-full bg-white text-black flex items-center justify-center"
          >
            <div className="p-4">Reveal All</div>
          </motion.button>
        )}
      </div>
    </>
